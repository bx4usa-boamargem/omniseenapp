...
async function executeOutlineGen(
  ...
  const contentType = ((jobInput.content_type as string) || 'article') as ContentTypeKey;
...
async function executeContentGenFromOutline(
  ...
  // Get content type template (structure, markers, quality rules)
  const template = getContentTypeTemplate(contentType, language);
  ...
  const prompt = `You are an elite SEO content strategist producing premium-quality articles. Write a FULL, in-depth article following this EXACT outline. Content type: ${contentType}.
...
async function executeQualityGate(
  ...
  // AI pattern detection (inspired by claude-blog burstiness analysis)
  const aiDetection = detectAiPatterns(html);
  ...
  if (!aiOk) reasons.push(`ai_patterns detected: ${aiDetection.flaggedPhrases.slice(0, 3).join(', ')} (score: ${aiDetection.score})`);
  ...
  return { passed, entityOk, wordOk, faqOk, scoreOk, aiOk, reasons, quality_gate_status: qualityGateStatus, ai_detection: aiDetection };
}
...
async function orchestrate(jobId: string, supabase: any, supabaseUrl: string, serviceKey: string): Promise<void> {
 try {
  ...
  try {
    ...
    const genStepId = await createStepOrFail(supabase, jobId, 'CONTENT_GEN', { keyword: jobInput.keyword, job_type: jobType, target_words: jobInput.target_words, content_type: jobInput.content_type || 'article' });
    ...
    try {
      ...
      await supabase.from('generation_steps').update({
        ...
        completed_at: new Date().toISOString(), model_used: retryResult.aiResult.model,
        provider: retryResult.aiResult.provider, cost_usd: retryResult.aiResult.costUsd,
      }).eq('id', genStepId);
    }
