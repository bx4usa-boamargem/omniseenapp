...
alter table articles
  add column if not exists geo_score              smallint     default null,
  add column if not exists geo_score_label        text         default null
      check (geo_score_label in ('poor','needs_work','good','excellent')),
  add column if not exists geo_score_breakdown    jsonb        default null,
  add column if not exists geo_score_passed       boolean      default null,
  add column if not exists geo_score_calculated_at timestamptz default null;
