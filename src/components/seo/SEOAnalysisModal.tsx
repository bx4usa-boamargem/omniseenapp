...
export function SEOAnalysisModal({
  ...
  const handleFixAll = async () => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error || "Erro ao otimizar SEO");
      }

      // Update local state with new values from API
      const updatedArticle = {
        ...currentArticle,
        title: data.updated.title,
        meta_description: data.updated.meta_description,
        content: data.updated.content_text,
      };
      setCurrentArticle(updatedArticle);

      // Use the score returned by the API
      const newResult = calculateSEOScore({
        title: data.updated.title,
        metaDescription: data.updated.meta_description || "",
        content: data.updated.content_text,
      ...
      const beforeScore = seoResult?.totalScore || 0;
      const afterScore = data.seo?.score_total || newResult.totalScore;
      const delta = afterScore - beforeScore;
      
      if (delta > 0) {
        toast.success(`SEO otimizado! Score: ${afterScore}% (+${delta} pontos)`);
      } else {
        toast.success(`SEO atualizado. Score: ${afterScore}%`);
      }
      
      onArticleUpdated();
    } catch (error) {
      console.error("Error fixing all SEO:", error);
      toast.error(error instanceof Error ? error.message : "Erro ao otimizar SEO");
    } finally {
      setFixingAll(false);
    }
  };
